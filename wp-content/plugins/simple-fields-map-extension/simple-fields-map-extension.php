...
function simple_fields_field_googlemaps_register() {
	...
	class simple_fields_field_googlemaps extends simple_fields_field {
		...
		function edit_output($saved_values, $options) {
			...
			$default_zoom_level = empty( $options["defaultZoomLevel"] ) ? 10 : $options["defaultZoomLevel"];
			$default_map_type_id = empty( $options["defaultMapTypeId"] ) ? "HYBRID" : $options["defaultMapTypeId"];
			...
			$output .= sprintf(
				...
				',
				$this->get_options_name("lat"), // 1
				esc_attr($lat_saved), // 2
				$this->get_options_name("lng"), // 3
				esc_attr($lng_saved), // 4
				round($lat_saved, 5), // 5
				round($lng_saved, 5), // 6
				$default_map_type_id, // 7
				empty($saved_preferred_zoom) ? $default_zoom_level : $saved_preferred_zoom, // use preferred over default
		 ...
		 */
		function return_values($values = null, $options = null) {
			...
			$static_map_base = "https://maps.googleapis.com/maps/api/staticmap?key=AIzaSyBPmFuKltNQP0KAmgWJZH1yyDBMNS4rPMQ";
